...
class Board(object):
    ...
    def make_move(self, col):
        ...
        if len(moves) == 0:
            ...
            return self.DRAW
        elif col - 1 not in moves:
            return self.ILLEGAL_MOVE

        self.state = self.get_new_board(col - 1)
    ...
    def next_state(self, state, play):
        print("Play: ", play)
        self.state = state
        return self.get_new_board(play - 1, state)
    ...
    def get_new_board(self, col, board=None):
        ...
        if col >= self.NCOL or col < 0:
            raise Exception("Illegal move requested!, col: ", col)
    ...
    def legal_plays(self, game_history = None):
        """
        ...
        """
        if game_history:
            self.state = game_history[-1]
        return [i + 1 for i in range(self.NCOL) if self._check_move(i)]
    ...
    def _check_move(self, col):
        ...
        if self.state[0, col - 1] == self.EMPTY:
            return True
        else:
            return False

    def winner(self, state_history):
        self.state = state_history[-1]
        if self._check_for_victory():
            return self.current_player(state_history)
        else:
            return 0

    def current_player(self, game_history):
        self.state = game_history[-1]
        if (np.sum(self.state) % 2 == 0):
            return 1
        else:
            return 2
